...
export default function ScreenPage() {
  ...
  useEffect(() => {
    if (loading) {
      ...
      setStatusMessage(
        activePlayer ? "Awaiting Human Input" : "Waiting for Combatant",
      );
    }
  ...
  const fetchData = async () => {
    ...
    try {
      const eventData: any = await getEvent(
        eventId,
              ...
              <div className="flex-1 flex gap-4 min-h-0">
                ...
                <div className="flex-1 bg-glass rounded-[32px] border border-white/10 relative overflow-hidden group flex flex-col">
                  <div className="absolute top-0 inset-x-0 h-10 bg-black/60 backdrop-blur-md flex items-center justify-between px-4 border-b border-primary/20 z-20">
                    <span className="text-[9px] font-orbitron font-black text-primary uppercase tracking-[0.3em]">
                      AI REFERENCE TARGET
                    </span>
                    <button
                      onClick={handleCopy}
                  ...
                  <div className="flex-1 relative">
                    {showQR ? (
                      ...
                      <AIImagePanel
                        ...
                        status={event.status}
                        description={event.referencePrompt}
                      />
                    )}
                  </div>
                </div>
                ...
                <div className="w-12 self-center flex flex-col items-center gap-2">
                  ...
                  <div className="w-10 h-10 rounded-full border border-primary/40 flex items-center justify-center bg-black rotate-45">
                    <span className="text-xs font-black font-orbitron text-primary -rotate-45 italic">
                      VS
                    </span>
                  </div>
                  <div className="h-20 w-px bg-gradient-to-b from-transparent via-primary/50 to-transparent" />
                ...
                <div className="flex-1 bg-glass rounded-[32px] border border-white/20 relative overflow-hidden group flex flex-col">
                  <div className="absolute top-0 inset-x-0 h-10 bg-black/60 backdrop-blur-md flex items-center justify-center border-b border-accent/20 z-20">
                    <span className="text-[9px] font-orbitron font-black text-accent uppercase tracking-[0.3em]">
              ...
              <div className="h-20 flex gap-4">
                <div className="flex-1 bg-glass rounded-2xl border border-white/5 flex items-center px-6 gap-4 relative overflow-hidden">
                  ...
                  <div className="w-10 h-10 rounded-xl bg-primary/20 border border-primary/30 flex items-center justify-center overflow-hidden">
                    {activePlayer ? (
                      <img
                        src={`https://api.dicebear.com/7.x/bottts/svg?seed=${activePlayer.id || activePlayer.name}`}
                  ...
                  <div className="flex-1">
                    <p className="text-[8px] font-orbitron font-black text-primary/40 uppercase tracking-widest">
                      Active Player
                    </p>
                    <AnimatePresence mode="wait">
                      <motion.p
                  ...
                  <div className="text-right">
                    <p className="text-[8px] font-orbitron font-black text-gold/40 uppercase tracking-widest">
                      Current Status
                    </p>
                    <p
                      className={`text-sm font-bold font-orbitron ${loading ? "text-primary" : "text-white/60"}`}
                    >
                      {loading
                        ? "PROCESSING..."
                        : activePlayer
                ...
                <div className="w-48 bg-glass rounded-2xl border border-white/5 flex items-center justify-around px-4">
                  <div className="text-center">
                    <p className="text-[7px] font-orbitron font-black text-primary/40 uppercase">
                      AI
                    </p>
                  ...
                  <div className="text-center">
                    <p className="text-[7px] font-orbitron font-black text-accent/40 uppercase">
                      Human
                    </p>
                    <p className="text-xl font-black font-orbitron text-white">
                      {event.humanWins || 0}
          ...
          >
            ...
            <form onSubmit={handleSendPrompt} className="flex-1 flex gap-3">
              <input
                ...
                placeholder={
                  activePlayer
                    ? "Enter your prompt to beat the AI..."
                    : "Waiting for player..."
                }
      ...
      <footer className="mt-auto py-4 flex justify-between items-center z-10 px-6 border-t border-white/5 bg-black/40 backdrop-blur-xl">
        <div className="flex gap-12">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-orbitron font-black text-primary/40 uppercase tracking-widest">
              AI WINS: {event.aiWins || 0}
            </span>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-1.5 h-1.5 bg-accent rounded-full animate-pulse shadow-[0_0_8px_rose]" />
            <span className="text-[10px] font-orbitron font-black text-accent/40 uppercase tracking-widest">
              HUMAN WINS: {event.humanWins || 0}
            </span>
          </div>
        </div>
